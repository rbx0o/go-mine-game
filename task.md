...
- Основной упор в предлагаемом учебном проекте делается на: интерфейсы, конкурентное взаимодействие на Golang, HTTP и REST API. В данный момент мы не рассматриваем лучшие практики организации кода и оптимальную архитектуру бекенд-приложения.
- Поэтому значительная часть лучших практик по организации кода не была использована в представленном коде. Первостепенная цель — повторение пройденного материала, а не демонстрация идеального написания Golang кода.
...
7. К найму доступно 3 класса шахтёров:
    ...
    - Сильный шахтёр:
        ...
        - Перерыв между добычами: 1 секунда
...
9. Все работающие шахтёры зависят от контекста предприятия, и предприятие имеет возможность в любой момент завершить работу всех шахтёров, не дожидаясь пока каждый отдельный шахтёр доработает свою очередную добычу.
...
11. Предприятие хранит всю информацию о шахтёрах, а именно:
    ...
    ```
12. Когда всё оборудование куплено, игрок должен дать предприятию команду к завершению. Если всё прошло успешно, предприятие должно отдать всю хранимую статистику, в том числе количество времени, понадобившееся на выполнение целей игры. К тому же, все запущенные шахтёры должны завершиться, запущенный HTTP сервер должен завершиться, после чего завершается и сама игра.
